...
#include "axmlprinter/axmlparse.h"
...
int extract_currentfile(unzFile zfile, char * extractdirectory)
{
	...
	if (strcmp(fileName_WithoutPath, "AndroidManifest.xml") == 0)
	{
		//判断当前文件是目录还是文件
		while ((*p) != '\0')
		{
		...
		else
		{
			...
			// 调用自己的axmlparse进行解析 [8/12/2017 ruiwei]
			AXMLParse *axmlparse = new AXMLParse();
			axmlparse->startParseFile(fileName_WithPath);
			axmlparse->writeFormatXmlToFile("extract/AndroidManifest-Format.xml");
			delete axmlparse;
			remove(fileName_WithPath);
		}
	}
	return 0;
}
